...
def prepare_dataloader(dataset, config, fn_preprocess=None, drop_remainder=True, repeat=False):
    ...
    if config.get("shuffle_buffer_size", 0):
        dataset = dataset.shuffle(buffer_size=config.shuffle_buffer_size, seed=shard_id)

    if fn_preprocess is not None:
        dataset = dataset.map(fn_preprocess, num_parallel_calls=tf.data.experimental.AUTOTUNE)
    ...
    dataset = dataset.batch(
        proc_batch_size // jax.local_device_count(), drop_remainder=drop_remainder
    )
    dataset = dataset.batch(jax.local_device_count(), drop_remainder=drop_remainder)
...
def tf_to_numpy(tf_batch):
    ...
    return jax.tree_util.tree_map(lambda x: x._numpy() if hasattr(x, "_numpy") else x, tf_batch)
