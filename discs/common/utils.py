...
def create_sharded_sampler_state(sampler_key, model, sampler, num_samples):
    ...
    sampler_states = [sampler.make_init_state(sampler_rng) for sampler_rng in sampler_rngs]
...
def graph2edges(
    ...
    if build_bidir_edges:
        ...
        if has_edge_weights:
            ret["bidir_edge_weight"] = jnp.concatenate((ret["edge_weight"], ret["edge_weight"]))
    ret["mask"] = None
    return ret
