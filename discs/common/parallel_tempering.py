...
def _vectorized_swap_step(
    new_x, energy, temperatures, indices_a, indices_b, rng_key
):
    energy_a, energy_b = energy[indices_a], energy[indices_b]
    temp_a, temp_b = temperatures[indices_a], temperatures[indices_b]
    
    log_prob_a, log_prob_b = -energy_a, -energy_b 
    log_acceptance_ratio = (1.0 / temp_a - 1.0 / temp_b) * (log_prob_b - log_prob_a) # log_acceptance_ratio shape: (num_swaps, batch_size)
...
def swap_samples_deo(new_x, energy, temperatures, rng_key, current_step):
    ...
    def perform_even_swap(operand_x):
        return _vectorized_swap_step( operand_x, energy, temperatures, even_indices_a, even_indices_b, swap_subkey )
    def perform_odd_swap(operand_x):
        return _vectorized_swap_step( operand_x, energy, temperatures, odd_indices_a, odd_indices_b, swap_subkey )
    
    func = perform_even_swap if do_even_swap else perform_odd_swap
    new_x, is_accept, new_rng_key = func(new_x)

    return new_x, is_accept

def swap_samples_seo(new_x, energy, temperatures, rng_key, current_step):
    # define the indices of the even and odd temperatures
    num_temperatures = temperatures.shape[0]
    even_indices_a = jnp.arange(0, num_temperatures - 1, 2)
    even_indices_b = jnp.arange(1, num_temperatures, 2)
