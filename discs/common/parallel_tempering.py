...
def _vectorized_swap_step(new_x, logprob, temperatures, indices_a, indices_b, rng_key):
    ...
    log_acceptance_ratio = (1.0 / temp_a - 1.0 / temp_b) * (logprob_b - logprob_a)
...
def swap_samples_deo(new_x, logprob, temperatures, rng_key, current_step):
    ...
    indices_a = jnp.arange(0, num_temperatures - 1, 2) + current_step % 2
    indices_b = jnp.arange(1, num_temperatures, 2) + current_step % 2
...
def swap_samples_seo(new_x, logprob, temperatures, rng_key, current_step):
    ...
    num_temperatures = temperatures.shape[0]
    indices_a = jnp.arange(0, num_temperatures - 1, 2) + do_odd_swap
    indices_b = jnp.arange(1, num_temperatures, 2) + do_odd_swap
...
def _swap_step(new_x, logprob, temperatures, indices_a, indices_b, rng_key):
    logprob_a, logprob_b = logprob[indices_a], logprob[indices_b]
    temp_a, temp_b = temperatures[indices_a], temperatures[indices_b]
...
def swap_samples_reversible(new_x, logprob, temperatures, rng_key, current_step):
    num_temperatures = temperatures.shape[0]
    num_swaps = num_temperatures - 1

    # 각 swap iteration마다 독립적인 키 분배
    swap_keys = jax.random.split(rng_key, num_swaps)

    acceptance_list = []
    indices_a_list = []
    indices_b_list = []
    for k, i in  enumerate(range(num_temperatures - 1, 0, -1)):
        new_x, swap_rate = _swap_step(new_x, logprob, temperatures, i, i - 1, swap_keys[k])
        acceptance_list.append(swap_rate)
        indices_a_list.append(i)
        indices_b_list.append(i - 1)

    # deo와 동일한 반환 형태: (num_swaps, batch) 텐서 + (num_swaps,) 인덱스 배열
    acceptance_ratio = jnp.stack(acceptance_list, axis=0)
