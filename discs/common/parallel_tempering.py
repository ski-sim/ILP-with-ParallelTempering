...
def _vectorized_swap_step(new_x, logprob, temperatures, indices_a, indices_b, rng_key):
    ...
    log_acceptance_ratio = (1.0 / temp_a - 1.0 / temp_b) * (
        logprob_b - logprob_a
    )  # log_acceptance_ratio shape: (num_swaps, batch_size)

    rng_key, subkey = jax.random.split(rng_key)
    is_accept = jax.random.uniform(subkey, shape=log_acceptance_ratio.shape) < jnp.exp(
        log_acceptance_ratio
    )  # is_accept shape: (num_swaps, batch_size)
...
def swap_samples_deo(new_x, logprob, temperatures, rng_key, current_step):
    ...
    def perform_even_swap(operand_x):
        return _vectorized_swap_step(
            operand_x, logprob, temperatures, even_indices_a, even_indices_b, swap_subkey
        )

    def perform_odd_swap(operand_x):
        return _vectorized_swap_step(
            operand_x, logprob, temperatures, odd_indices_a, odd_indices_b, swap_subkey
        )
