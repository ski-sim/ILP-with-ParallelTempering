...
class ILPGraphGen():
  ...
  def __init__(self, data_root, model_config):
    super().__init__()
    data_folder = os.path.join(data_root, '%s_%d' % (model_config.graph_type, model_config.max_num_nodes))
  ...
  def get_iterator(self, phase, batch_size, sharding=False):
    ...
    if self.num_instances % batch_size != 0:
      total_batches += 1
    generator = self.sample_gen(phase, repeat=False) # graphs generator outpust the graph and obj
    num_batches = 0
    buffer = []
      ...
      if idx % num_proc == proc_idx:
        try:
          ...
          params = {
              ...
              'var_types': jnp.array(var_types)  # 변수 타입: 0=BINARY, 1=INTEGER, 3=CONTINUOUS
          }
