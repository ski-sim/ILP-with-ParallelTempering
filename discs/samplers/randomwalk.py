...
class RandomWalkSampler(abstractsampler.AbstractSampler):
    ...
    def step(self, model, rng, x, model_param, state, x_mask=None):
        ...
        ll_x, is_valid_x = model.forward(model_param, x)
        y = self.sample_from_proposal(rng_new_sample, x, state)
        ll_y, is_valid_y = model.forward(model_param, y)
        log_acc = ll_y - ll_x
        new_x, new_state = self.select_sample(
            rng_acceptance, log_acc, x, y, ll_x, ll_y, is_valid_x, is_valid_y, state
        )
    ...
    def sample_from_proposal(self, rng_new_sample, x, state):
        ...
        indices_to_flip = random.bernoulli(rnd_new_sample, p=(state["radius"] / dim), shape=x.shape)
...
class RWSampler(RandomWalkSampler):
    ...
    def select_sample(
        ...
        sampler_state["log_prob"] = jnp.where(acc, new_ll, current_ll)
        sampler_state["is_valid"] = jnp.where(acc, new_is_valid, current_is_valid)
