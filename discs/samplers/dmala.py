...
class DMALASampler(locallybalanced.LocallyBalancedSampler):
    ...
    def get_value_and_rates(self, model, model_param, x):
        grad_x, (ll_x, is_valid_x) = model.get_value_and_grad(model_param, x)
        if self.num_categories == 2:
        ...
        return ll_x, {"weights": log_weight_x, "delta": ll_delta}, is_valid_x
    ...
    def step(self, model, rng, x, model_param, state, x_mask=None):
        ...
        ll_x, log_rate_x, is_valid_x = self.get_value_and_rates(model, model_param, x)
        if self.adaptive:
            local_stats = self.reset_stats(log_rate_x["weights"])
            log_tau = jnp.where(state["steps"] == 0, local_stats["log_tau"], state["log_tau"])
        ...
        ll_y, log_rate_y, is_valid_y = self.get_value_and_rates(model, model_param, y)
        ...
        new_x, new_state = self.select_sample(
            rng_acceptance, local_stats, log_acc, x, y, ll_x, ll_y, is_valid_x, is_valid_y, state
        )
