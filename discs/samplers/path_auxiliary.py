...
class PAFSNoReplacement(PathAuxiliarySampler):
    ...
    def proposal(self, model, rng, x, model_param, state, x_mask):
        ...
        if self.adaptive:
            num_samples = jnp.clip(
                jnp.round(state["radius"]).astype(jnp.int32),
                a_min=1,
                a_max=math.prod(self.sample_shape),
            )
        else:
        ...
        selected_idx, ll_selected = math.multinomial(
            ...
            replacement=False,
            return_ll=True,
            is_nsample_const=not self.adaptive,
            need_ordering_info=True,
        )
        if self.adaptive:
    ...
    def ll_y2x(self, model, x, model_param, forward_trajectory, y, x_mask):
        ...
        if self.adaptive:
            ...
            if self.num_categories > 2:
                ...
                x = jnp.reshape(x, [x.shape[0], -1, self.num_categories])
                orig_val = x[jnp.expand_dims(jnp.arange(x.shape[0]), axis=1), reverse_idx_traj]
