...
def _vectorized_swap_step(new_x, P_x, temperature, penalty_coeffs, indices_a, indices_b, rng_key):
    # P(x) = Sum (Ax - b);  P_x[i] = -lambda_i * P(x_i)
    penalty_coeff_a, penalty_coeff_b = penalty_coeffs[indices_a], penalty_coeffs[indices_b]
    P_a, P_b = P_x[indices_a] / penalty_coeff_b, P_x[indices_b] / penalty_coeff_a

    # beta * Delta lambda * (P_b - P_a)
    log_acc = (1 / temperature) * (penalty_coeff_b - penalty_coeff_a) * (P_b - P_a)
...
def swap_samples_reversible(new_x, logprob, obj_coeff, temperature, penalty_coeffs, rng_key, current_step):
    ladder_size = penalty_coeffs.shape[0]
    ...
    for k, i in enumerate(range(ladder_size - 1, 0, -1)):
        new_x, swap_rate = _vectorized_swap_step(
            new_x, P_x, temperature, penalty_coeffs, i, i - 1, swap_keys[k]
        )
