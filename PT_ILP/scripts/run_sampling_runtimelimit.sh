...
formulation=${7:-max_linear_square}

valid_schedules="[exp_decay, pt, pt_exp_decay, pen_pt, pen_pt_exp_decay]"
case "$t_schedule" in
   exp_decay|pt|pt_exp_decay|pen_pt|pen_pt_exp_decay) ;;
   *)
...
taskset -c 0-31 python -W ignore -m PT_ILP.main_sampling \
   ...
   --l_min=$(echo "${penalty_weight}/2" | bc -l) --l_max=${penalty_weight} \
   --t_schedule=${t_schedule} --init_temperature=${init_temperature} --decay_rate=0.5 \
   --pt=deo --pt_interval=200 --t_min=${init_temperature} --t_max=$(echo "2*${init_temperature}" | bc -l) --log_every_steps=100 --mode=runtimelimit --max_runtime=200.0
